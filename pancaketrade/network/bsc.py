...
class Network:
    ...
    def get_best_swap_path(
        ...
        if not valid_paths:
            raise ValueError('Не найдена действующая пара')
    ...
    def buy_tokens(
        ...
        if amount_bnb > balance_bnb - Wei(2000000000000000):  # leave 0.002 BNB for future gas fees
            logger.error('Не достаточный баланс BNB ')
            return False, Decimal(0), 'Не достаточный баланс BNB '
        ...
        try:
            ...
            return (
                False,
                Decimal(0),
                'Не найдено ни одного совместимого LP',
            )
        ...
        if receipt is None:
            logger.error('Не могу получить оценку газа')
            return (
                False,
                Decimal(0),
                f'Не удается получить оценку газа, проверьте правильность настройки проскальзывания (текущее {slippage_percent}%)',
            )
        txhash = Web3.toHex(primitive=receipt["transactionHash"])
        if receipt['status'] == 0:  # fail
            logger.error(f'Ошибка транзакции покупки в tx {txhash}')
        ...
        for log in reversed(logs):  # only get last withdrawal call
            ...
            break
        logger.success(f'Транзакция покупки успешна в tx {txhash}')
    ...
    def buy_tokens_with_params(
        ...
        try:
            ...
            logger.error(f'Не удается получить оценку газа, отмена транзакции: {e}')
            return None
    ...
    def sell_tokens(
        ...
        txhash = Web3.toHex(primitive=receipt["transactionHash"])
        if receipt['status'] == 0:  # fail
            logger.error(f'Ошибка транзакции продажи в tx {txhash}')
            return False, Decimal(0), txhash
        amount_out = Decimal(0)
        ...
        for log in reversed(logs):  # only get last withdrawal call
            ...
            break
        logger.success(f'Транзакция продажи успешна в tx {txhash}')
        return True, amount_out, txhash
    ...
    def approve(self, token_address: ChecksumAddress, max_approval: Optional[int] = None) -> bool:
        ...
        logger.info(f'Подтверждение {self.get_token_symbol(token_address=token_address)} - {token_address}...')
        try:
            gas_limit = Wei(int(Decimal(func.estimateGas({'from': self.wallet, 'value': Wei(0)})) * Decimal(1.2)))
        except Exception:
        ...
        if receipt['status'] == 0:  # fail
            logger.error(f'Вызов подтверждения не состоялся в tx {Web3.toHex(primitive=receipt["transactionHash"])}')
            return False
        self.approved.add(str(token_address))
        time.sleep(3)  # let tx propagate
        logger.success('Подтвержден кошелек для торговли.')
        return True
