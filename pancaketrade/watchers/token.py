    ...
    def monitor_price(self):
        ...
        for i, order in enumerate(self.orders):
            if order.finished:
                ...
                continue
            if not self.net.is_approved(token_address=self.address) and order.type == 'Продажа':
                # when selling we require that the token is approved on pcs beforehand
                logger.info(f'Требуется подтверждение {self.symbol} для торговли на PancakeSwap.')
                self.dispatcher.bot.send_message(
                    chat_id=self.config.secrets.admin_chat_id,
                    text=f'Подтверждение {self.symbol} для торговли на PancakeSwap...',
                )
