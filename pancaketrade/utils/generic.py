...
def check_chat_id(func: Callable) -> Callable:
    ...
    def wrapper_check_chat_id(this, update: Update, context: CallbackContext, *args, **kwargs):
        ...
        logger.warning(f'Запрет на взаимодействие с пользователем {chat_id} .')
        context.bot.send_message(
            chat_id=this.config.secrets.admin_chat_id, text=f'Запрет на взаимодействие с пользователем {chat_id} .'
        )
        context.bot.send_message(chat_id=chat_id, text='Этот бот не является публичным, вам не разрешено его использовать.')
    ...
    if update.callback_query is not None and edit:
        ...
        try:
            ...
            if not str(e).startswith('Сообщение не изменено'):
                ...
                context.bot.send_message(chat_id=update.effective_chat.id, text=f'Исключение при обновлении сообщения: {e}')
        return None
...
def get_tokens_keyboard_layout(
    ...
    buttons.append(InlineKeyboardButton('❌ Отмена', callback_data='canceltokenchoice'))
