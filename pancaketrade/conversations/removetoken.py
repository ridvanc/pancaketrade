...
class RemoveTokenConversation:
    ...
    def command_removetoken(self, update: Update, context: CallbackContext):
        ...
        if not Web3.isChecksumAddress(token_address):
            chat_message(update, context, text='⛔️ Неверный адрес токена.', edit=self.config.update_messages)
            return ConversationHandler.END
        token = self.parent.watchers[token_address]
        chat_message(
            update,
            context,
            text=f'Вы уверены что хотите удалить {token.name}?',
    ...
    def command_removetoken_confirm(self, update: Update, context: CallbackContext):
        ...
        chat_message(
            update,
            context,
            text=f'✅ Хорошо, токен <b>"{token_name}"</b> был удален.',
            edit=self.config.update_messages,
        )
        return ConversationHandler.END
