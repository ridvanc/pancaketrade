...
class AddOrderConversation:
    ...
    def command_addorder(self, update: Update, context: CallbackContext):
        ...
        if not Web3.isChecksumAddress(token_address):
            self.command_error(update, context, text='Неверный адрес токена.')
        ...
        reply_markup = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton('🚫 Stop loss продажа', callback_data='stop_loss'),
                    InlineKeyboardButton('💰 Take profit продажа', callback_data='limit_sell'),
                ],
                [
                    InlineKeyboardButton('💵 Лимитная покупка', callback_data='limit_buy'),
        ...
        chat_message(
            update,
            context,
            text=f'Создание ордера для токена {token.name}.\nКакой <u>type of order</u> вы бы хотели создать?',
    ...
    def command_addorder_type(self, update: Update, context: CallbackContext):
        ...
        if query.data == 'stop_loss':
            ...
            chat_message(
                update,
                context,
                text='OK, ордер будет продан, как только цена станет ниже целевой цены.\n'
        ...
        reply_markup = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    InlineKeyboardButton('Без trailing stop loss', callback_data='None'),
                    InlineKeyboardButton('❌ Отмена', callback_data='cancel'),
                ],
            ]
        )
    ...
    def command_addorder_trailing(self, update: Update, context: CallbackContext):
        ...
        if update.message is None:
            ...
            try:
                ...
                chat_message(update, context, text='⚠️ The callback rate is not recognized, попробуйте снова:', edit=False)
                return self.next.TRAILING
        order['trailing_stop'] = callback_rate
        chat_message(
            update,
            context,
            text=f'OK, ордер будет использовать trailing stop loss с {callback_rate}% callback.\n' + next_message,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('❌ Cancel', callback_data='cancel')]]),
            edit=self.config.update_messages,
        )
        return self.next.PRICE
    ...
    def command_addorder_price(self, update: Update, context: CallbackContext):
        ...
        if answer.endswith('x'):
            try:
                ...
                chat_message(update, context, text='⚠️ The factor you inserted is not valid. Попробуйте снова:', edit=False)
                return self.next.PRICE
            current_price, _ = self.net.get_token_price(token_address=token.address)
            ...
            try:
                ...
                chat_message(update, context, text='⚠️ The price you inserted is not valid. Попробуйте снова:', edit=False)
                return self.next.PRICE
    ...
    def command_addorder_amount(self, update: Update, context: CallbackContext):
        ...
        if update.message is None:  # we got a button callback, either cancel or fraction of token balance
            ...
            if user_input.endswith('%'):
                try:
                    ...
                    chat_message(
                        update, context, text='⚠️ The balance percentage is not recognized, попробуйте снова:', edit=False
                    )
                    return self.next.AMOUNT
            else:
                try:
                    ...
                    chat_message(
                        update, context, text='⚠️ The amount you inserted is not valid. Попробуйте снова:', edit=False
                    )
                    return self.next.AMOUNT
        ...
        chat_message(
            ...
            + 'Далее, пожалуйста укажите <u>проскальзывание в процентах</u> которое вы хотите использовать для этого ордереа.\n'
            + 'Вы так же можете отправить мне сообщение в процентах.',
            reply_markup=reply_markup,
            edit=self.config.update_messages,
        )
    ...
    def command_addorder_slippage(self, update: Update, context: CallbackContext):
        ...
        if update.message is None:
            ...
            try:
                ...
                chat_message(update, context, text='⚠️ Проскальзывание не подтверждено, попробуйте снова:', edit=False)
                return self.next.SLIPPAGE
        if slippage_percent < Decimal("0.01") or slippage_percent > 100:
            chat_message(update, context, text='⚠️ Проскальзование должно быть между 0.01 и 100, попробуйте снова:', edit=False)
            return self.next.SLIPPAGE
        ...
        chat_message(
            update,
            context,
            text=f'OK, ордер будет использовать проскальзывание {slippage_percent}%.\n'
            + 'Наконец, пожалуйста укажите <u>gas price in Gwei</u> для этого ордера.\n'
            + 'Используйте "Default" чтобы использовать default network в данный момент '
            + f'для транзакции (текущий {network_gas_price:.1f} Gwei) '
        ...
        if update.message is None:
            ...
            if query.data == 'cancel':
                ...
                chat_message(
                    update,
                    context,
                    text='OK, ордер будет испорльзовать default network gas price.\nПодтвердите ордер ниже!',
                    edit=self.config.update_messages,
                )
                ...
                chat_message(
                    update,
                    context,
                    text=f'OK, ордер будет использовать default network gas price {query.data} Gwei.\n'
                    + 'Подтвердите ордер ниже!',
                    edit=self.config.update_messages,
                )
            else:
            ...
            try:
                ...
                chat_message(update, context, text='⚠️ The gas price не распознана, попробуйте снова:', edit=False)
                return self.next.GAS
        order['gas_price'] = str(Web3.toWei(gas_price_gwei, unit='gwei'))
        chat_message(
            update,
            context,
            text=f'OK, ордер будет использовать {gas_price_gwei:.4g} Gwei для gas price.\n<u>Подтвердите</u> ордер ниже!',
            edit=self.config.update_messages,
        )
        return self.print_summary(update, context)
    ...
    def print_summary(self, update: Update, context: CallbackContext):
        ...
        message = (
            ...
            + f'Цена {comparision} {limit_price:.3g} BNB за токен\n'
            + f'Проскальзывание: {order["slippage"]}%\n'
            + f'Gas: {gas_price}'
        )
        chat_message(
            update,
    ...
    def command_addorder_summary(self, update: Update, context: CallbackContext):
        ...
        chat_message(
            update,
            context,
            text=f'✅ Ордер #{order_record.id} был успешно добавлен!',
            edit=self.config.update_messages,
        )
    ...
    def get_price_message(self, current_price: Decimal, token_symbol: str) -> str:
        ...
        next_message = (
            f'Далее, пожалуйста укажите <u>цена в <b>BNB за {token_symbol}</b></u> '
            + 'по которой ордер будет активирован.\n'
            + 'У вас есть 3 способа для этого:\n'
            + f' ・ Обычная запись например "<code>{current_price_fixed}</code>"\n'
            + f' ・ Научная запись например "<code>{current_price:.1e}</code>"\n'
            + ' ・ Коэфициент умножения например "<code>1.5x</code>" (include the "x" at the end)\n'
            + f'<b>Текущая цена</b>: <code>{current_price:.4g}</code> BNB за {token_symbol}.'
        )
        return next_message
