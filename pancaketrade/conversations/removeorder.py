...
class RemoveOrderConversation:
    ...
    def command_removeorder(self, update: Update, context: CallbackContext):
        ...
        chat_message(
            update,
            context,
            text=f'Выберете ордер который вы хотите удалить для токена {token.name}.\n\n' + '\n'.join(orders_display),
    ...
    def command_removeorder_confirm(self, update: Update, context: CallbackContext):
        ...
        if not query.data.isdecimal():
            self.command_error(update, context, text='Не верный ID ордера')
            return ConversationHandler.END
        token: TokenWatcher = self.parent.watchers[context.user_data['removeorder']['token_address']]
        chat_message(
            update,
            context,
            text=f'Вы уверены что хотите удалить ордер #{query.data} для {token.name}?',
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('✅ Подтвердить', callback_data=query.data),
    ...
    def command_removeorder_order(self, update: Update, context: CallbackContext):
        ...
        try:
            ...
            self.command_error(update, context, text=f'Ордер {query.data} не может быть найден.')
            return ConversationHandler.END
        ...
        chat_message(
            update,
            context,
            text=f'✅ Хорошо, ордер <b>#{query.data}</b> был удален для {token.name}.',
            edit=self.config.update_messages,
        )
        return ConversationHandler.END
